...
module Jekyll
  ...
  module Sanitizer
    ...
    def sanitize_filename(name)
      ...
      ).downcase.strip.gsub(' ', '-').gsub(/-{2,}/, '-').gsub(/[^\w.-]/, '').to_s
    end
  end
  ...
  class DataPage < Page
    ...
    def initialize(site, base, index_files, dir, data, subfolders, name_prefix, name, template, extension)
      @site = site
      @base = base
      ...
      if name_prefix
        filename = sanitize_filename(name_prefix).to_s + "-" + filename
      end
      if subfolders
        subfolders = subfolders.map{ |ref| sanitize_filename(data[ref]) }.join('/')
        dir = dir + "/" + subfolders.to_s
      end
      if index_files
    ...
    def generate(site)
      ...
      if data
        data.each do |data_spec|
          ...
          subfolders = data_spec['subfolders']
          name_prefix = data_spec['name_prefix']
          name = data_spec['name']
          ...
          if site.layouts.key? template
            ...
            records.each do |record|
              site.pages << DataPage.new(site, site.source, index_files, dir, record, subfolders, name_prefix, name, template, extension)
            end
          else
